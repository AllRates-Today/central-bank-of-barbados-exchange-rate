# Central Bank of Barbados Exchange Rates API — central-bank-of-barbados-exchange-rate

[![npm version](https://img.shields.io/npm/v/central-bank-of-barbados-exchange-rate.svg)](https://www.npmjs.com/package/central-bank-of-barbados-exchange-rate)
...
- 🏛️ **Official published rates** — Central Bank of Barbados's own table, with the publisher's own `rate_date` on every response
...
> **Official rate, not mid-market:** every value here is a number Central Bank of Barbados itself published, fixed once printed and carrying the central bank's own `rate_date` — what filings and audits require. Need the live interbank midpoint for pricing or display instead? Use the [mid-market API](https://allratestoday.com/docs/) or [`@allratestoday/sdk`](https://www.npmjs.com/package/@allratestoday/sdk). The two can diverge by several percent.

## ⚡ Try it without a key

The latest Central Bank of Barbados table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/cbbd?source=USD&target=BBD"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/cbbd').then((x) => x.json());
...
```bash
npm install central-bank-of-barbados-exchange-rate
```

```bash
yarn add central-bank-of-barbados-exchange-rate
```

```bash
pnpm add central-bank-of-barbados-exchange-rate
```

Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/central-bank-of-barbados-exchange-rate`](https://www.npmjs.com/package/@allratestoday/central-bank-of-barbados-exchange-rate) — same code, same versions.

## 🏁 Quick start

```js
import { getRate } from 'central-bank-of-barbados-exchange-rate';

const pair = await getRate('USD', 'BBD', { apiKey: 'art_live_...' });
console.log(pair.rate, pair.rate_date); // the official Central Bank of Barbados rate, on the central bank's own date
```
...
```js
import { getLatestRates } from 'central-bank-of-barbados-exchange-rate';
...
```javascript
{
  ...
  rates: [
    ...
    // … the rest of the published table (7 currencies vs BBD)
  ],
  disclaimer: '…'
}
```
...
```js
import { getRatesForDate } from 'central-bank-of-barbados-exchange-rate';

const day = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...' });
// Optionally narrow to one pair:
const one = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...', source: 'USD', target: 'BBD' });
```
...
Central Bank of Barbados currently publishes rates covering **7 currencies** against the BBD (as of the latest table):

🇧🇿 `BZD` · 🇨🇦 `CAD` · 🇪🇺 `EUR` · 🇬🇧 `GBP` · 🇬🇾 `GYD` · 🇺🇸 `USD` · `XCD`

## 🏛️ Source

The Central Bank of Barbados publishes daily buying and selling rates for the US dollar, pound sterling, euro, Canadian dollar and the neighbouring Caribbean currencies, in Barbados dollars per unit. The Barbados dollar is pegged at two to the US dollar, so the table is mostly about the cross rates — and the bank's archive runs back to 2010.

- Publisher's own page: [Exchange rates](https://www.centralbank.org.bb/exchange-rates) · [www.centralbank.org.bb](https://www.centralbank.org.bb)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Central Bank of Barbados rates page](https://allratestoday.com/central-bank-rates-api/cbbd/)
...
- Central Bank of Barbados quotes **BBD per 1 unit of foreign currency** (e.g. `base: "USD", quote: "BBD"` means BBD per one US dollar).
...
Loading the official Central Bank of Barbados rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/cbbd/latest?format=xml&api_key=art_live_..."
```
...
If Central Bank of Barbados does not print a pair directly, the API resolves it from the central bank's own table and says so — official and computed values are never confused:

| `method` | `derived` | Meaning |
| --- | --- | --- |
| `published` | `false` | The central bank printed this pair directly |
| `inverse` | `true` | Computed as 1 ÷ the published opposite direction |
| `cross` | `true` | Computed via BBD from two published rates |
...
| `404` | Pair or date range not covered by Central Bank of Barbados |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'central-bank-of-barbados-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('central-bank-of-barbados-exchange-rate');

getRate('USD', 'BBD', { apiKey: 'art_live_...' }).then((pair) => console.log(pair.rate));
```
...
- [Central Bank of Barbados rates page](https://allratestoday.com/central-bank-rates-api/cbbd/) — live table, publication cadence, FAQ
- [All central bank sources](https://allratestoday.com/central-bank-rates-api/)
- [Package docs on the site](https://allratestoday.com/docs/sdk/central-bank-of-barbados-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
...
- [GitHub](https://github.com/AllRates-Today/central-bank-of-barbados-exchange-rate)
